...
→ build → package the asset → **only now** write the version + commit + push →
`gh release create --latest <asset> <app-info.json>`. `chmod +x`. Traps: see
...
2. **A read-only token embedded at build time** — only when the repo is **private**. Keep the
   secret file gitignored (`packaging/app_secret.py`) and embed it into **every** build: the
...
- `./run.sh build` — a real build; open it if it's an `.app`.
- For an `.app`: the embedded version is right, the token is there, `codesign -dv` is quiet.
