...
| Skill | What it does |
|---|---|
| [`core-scripts-setup`](skills/core-scripts-setup/) | Scaffolds `setup.sh` + `run.sh` + `release.sh` + local install + auto-update infrastructure over GitHub Releases, for a project in any language. Ships templates that have run in production for macOS `.app` install/self-update. |
